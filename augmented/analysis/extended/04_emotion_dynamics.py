"""Analyzes how emotion state modulates keystroke dynamics.

Tests whether entropy, mean IKI, and autocorrelation differ systematically across
anger, happiness, calmness, sadness, and neutral. Paper relevance: validates cognitive
state -> process signal mapping.

Usage:
    uv run python analysis/extended/04_emotion_dynamics.py \\
        --data-dir data/ -o results/emotion_dynamics.json
"""
...
def main():
    ...
    parser.add_argument("-o", "--output", default="results/emotion_dynamics.json")
    ...
    for code, user_dict in sorted(raw.items()):
    ...
    for code, user_dict in raw.items():
        label = EMOTION_LABELS.get(code, code)
