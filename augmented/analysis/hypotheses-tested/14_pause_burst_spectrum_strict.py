"""Pause-burst dynamics with task-type stratification.

HYPOTHESIS: Part of spectrum test. Pause/burst ratio as cognitive load indicator (process-intrinsic).
STATUS: Tested and falsified. See SPECTRUM_HYPOTHESIS_LOG.md

Quantifies pause-burst dynamics across datasets. Original analysis aggregates all task types
(transcription vs composition), which confounds the results. Pause ratio differs dramatically
by task type; stratification reveals the true within-task signal without mixing fundamentally
different writing processes.

Paper relevance: provides a simple, interpretable signal for the cognitive spectrum claim.

Usage:
    uv run python hypotheses-tested/14_pause_burst_spectrum_strict.py \\
        --data-dir data/ -o results/pause_burst_spectrum_strict.json
"""
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", default="results/pause_burst_spectrum_strict.json")
    ...
    _out("Stratifying by task type to avoid confounding...")
    ...
    per_task_summary = {
        tt: {"mean_ratio": round(_mean(vals), 6), "std_ratio": round(_std(vals), 6), "n": len(vals)}
        for tt, vals in per_task_type.items()
    }

    _out(f"\nPer-task-type pause/burst ratio (stratified analysis):")
    for tt in sorted(per_task_summary.keys()):
        d = per_task_summary[tt]
        _out(f"  {tt:20s}  mean={d['mean_ratio']:.4f}  std={d['std_ratio']:.4f}  n={d['n']}")
    ...
    result = {
        ...
        "n_task_types": len(per_task_summary),
        ...
        "hypothesis": "Pause/burst as cognitive load indicator (spectrum test)",
        "stratification_note": "Analysis stratified by task type to control for confounding between transcription and composition",
    }
