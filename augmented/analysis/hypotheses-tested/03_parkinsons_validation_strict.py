"""Parkinson's validation with minimum sample size enforcement.

HYPOTHESIS: Part of spectrum test. Keystroke signals for clinical detection (state-intrinsic).
STATUS: Tested and falsified. See SPECTRUM_HYPOTHESIS_LOG.md

Filters to only subjects with ≥500 IKI values and requires ≥30 subjects per group.
This avoids underpowered comparisons and spurious null results.

Usage:
    uv run python hypotheses-tested/03_parkinsons_validation_strict.py \\
        --data-dir data/ -o results/parkinsons_validation_strict.json
"""
...
def main():
    ...
    parser.add_argument("-o", "--output", default="results/parkinsons_validation_strict.json")
    ...
    MIN_PER_GROUP = 30
    _out(f"  Before filtering: PD={n_pd}, healthy={n_healthy}, skipped={n_skipped}, insufficient_data={n_filtered_insufficient}")
    _out(f"  Requiring minimum {MIN_PER_GROUP} per group for adequate power...")

    if n_pd < MIN_PER_GROUP or n_healthy < MIN_PER_GROUP:
        _out(f"WARNING: insufficient data after filtering (PD={n_pd}, H={n_healthy})")
        args.output.parent.mkdir(parents=True, exist_ok=True)
        json.dump({
            "n_pd": n_pd,
            "n_healthy": n_healthy,
            "minimum_required_per_group": MIN_PER_GROUP,
            "note": "Hypothesis test (spectrum): Parkinson's is state-level property. Insufficient data.",
            "signals": {},
    ...
    result = {
        "n_pd": n_pd,
        "n_healthy": n_healthy,
        "minimum_per_group_threshold": MIN_PER_GROUP,
        "min_iki_per_subject": 500,
        "hypothesis": "Parkinson's detection is state-level property (spectrum test)",
