"""Tests whether process signals form a biometric fingerprint with strict data thresholds.

HYPOTHESIS: Part of spectrum test. Writer identification via keystroke patterns (state-intrinsic).
STATUS: Tested and falsified. See SPECTRUM_HYPOTHESIS_LOG.md

Uses 1-nearest-neighbor identification on KLiCKe writers. Filters to writers with ≥1000 IKI
values in each half-session. Original dataset has many writers with <100 keystroke samples,
which produces spurious matches on thin data.

Each writer's keystroke log is split into a first-half probe and second-half gallery entry.
Cross-half 1-NN identification is then performed: for writer i, find the gallery entry whose
feature vector is nearest to the probe.

Usage:
    uv run python hypotheses-tested/06_writer_identification_strict.py \\
        --data-dir /path/to/data -o results/06_writer_identification_strict.json
"""
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", type=Path, default=Path("06_writer_identification_strict.json"))
    ...
    if not csv_dir.exists():
        ...
        json.dump({"n_writers": 0, "n_writers_qualified": 0, "min_iki_per_half": MIN_IKI_PER_HALF,
                   "rank1_accuracy": None, "rank5_accuracy": None, "rank10_accuracy": None,
                   "chance_rank1": None, "chance_rank5": None, "chance_rank10": None,
                   "identification_entropy_bits": None, "above_chance_ratio": None,
                   "hypothesis": "Writer identification (spectrum test)"}, open(args.output, "w"), indent=2)
        return

    csv_files = sorted(csv_dir.glob("*.csv"))
    _out(f"Processing {len(csv_files)} writer CSV files...")
    _out(f"Requiring minimum {MIN_IKI_PER_HALF} IKI values per half-session...")
    ...
    for i, csv_path in enumerate(csv_files):
        ...
        # Extract IKI counts from half-sessions to verify minimum
        iki_a: List[float] = []
        iki_b: List[float] = []
        for row in all_rows[:mid]:
            try:
                down_time = float(row.get("DownTime") or 0)
                if down_time > 0:
                    if iki_a and down_time > iki_a[-1]:
                        iki_val = down_time - iki_a[-1]
                        if IKI_MIN_MS < iki_val < IKI_MAX_MS:
                            iki_a.append(iki_val)
                    iki_a.append(down_time)
            except (ValueError, TypeError):
                continue
        for row in all_rows[mid:]:
            try:
                down_time = float(row.get("DownTime") or 0)
                if down_time > 0:
                    if iki_b and down_time > iki_b[-1]:
                        iki_val = down_time - iki_b[-1]
                        if IKI_MIN_MS < iki_val < IKI_MAX_MS:
                            iki_b.append(iki_val)
                    iki_b.append(down_time)
            except (ValueError, TypeError):
                continue

        if len(iki_a) < MIN_IKI_PER_HALF or len(iki_b) < MIN_IKI_PER_HALF:
    ...
    n = len(half_a)
    _out(f"  Usable writers: {n}  (skipped {skipped}, insufficient_data {skipped_insufficient})")
    if n < 10:
        _out(f"WARNING: Too few writers for split evaluation ({n}); skipping analysis.")
        args.output.parent.mkdir(parents=True, exist_ok=True)
        json.dump({"n_writers": len(csv_files), "n_writers_qualified": n, "min_iki_per_half": MIN_IKI_PER_HALF,
                   "rank1_accuracy": None, "rank5_accuracy": None, "rank10_accuracy": None,
                   "chance_rank1": None, "chance_rank5": None, "chance_rank10": None,
                   "identification_entropy_bits": None, "above_chance_ratio": None,
                   "hypothesis": "Writer identification (spectrum test)"}, open(args.output, "w"), indent=2)
    ...
    _out(f"\nResults (n={n} writers, >={MIN_IKI_PER_HALF} IKI per half):")
    ...
    output = {
        "n_writers": len(csv_files),
        "n_writers_qualified": n,
        "min_iki_per_half": MIN_IKI_PER_HALF,
        "hypothesis": "Writer identification (spectrum test)",
