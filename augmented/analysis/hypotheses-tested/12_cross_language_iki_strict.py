"""Cross-language IKI analysis with minimum participant threshold per nationality.

HYPOTHESIS: Part of spectrum test. Language-universal process signals (process-intrinsic).
STATUS: Tested and falsified. See SPECTRUM_HYPOTHESIS_LOG.md

Analyzes whether typing language/nationality influences IKI distributions. Filters to
nationalities with ≥15 participants. Original data has many nationalities with n=2-3,
which are uninformative for statistical comparison.

Paper relevance: if IKI is language-universal, process signals are universal cognitive
properties not linguistic artifacts.

Usage:
    uv run python hypotheses-tested/12_cross_language_iki_strict.py \\
        --data-dir data/ -o results/cross_language_iki_strict.json
"""
...
IKI_MIN_MS = 10.0
IKI_MAX_MS = 30_000.0
MIN_PARTICIPANTS_PER_NATIONALITY = 15
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", default="results/cross_language_iki_strict.json")
    ...
    _out(f"Requiring minimum {MIN_PARTICIPANTS_PER_NATIONALITY} participants per nationality...")

    qualified_nats = {
        nat for nat, cnt in nat_participant_counts.items()
        if cnt >= MIN_PARTICIPANTS_PER_NATIONALITY
    }
    _out(f"  Total nationalities: {len(nat_participant_counts)}")
    _out(f"  Nationalities with >= {MIN_PARTICIPANTS_PER_NATIONALITY} participants: {len(qualified_nats)}")

    if len(qualified_nats) < 2:
        _out(f"WARNING: insufficient nationalities after filtering (need >=2 for comparison)")
        json.dump({
            "n_nationalities_total": len(nat_participant_counts),
            "n_nationalities_qualified": len(qualified_nats),
            "minimum_participants_per_nationality": MIN_PARTICIPANTS_PER_NATIONALITY,
            "note": "Hypothesis test (spectrum): language universality. Insufficient data after strict filtering.",
            "per_nationality": {},
    ...
    result = {
        "n_nationalities_total": len(nat_participant_counts),
        "n_nationalities_qualified": len(per_nationality),
        "minimum_participants_per_nationality": MIN_PARTICIPANTS_PER_NATIONALITY,
        "n_participants_total": sum(d["n"] for d in per_nationality.values()),
        "hypothesis": "Language-universal process signals (spectrum test)",
