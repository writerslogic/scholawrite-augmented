"""Fatigue trajectory validation with within-writer slope filtering.

HYPOTHESIS: Part of spectrum test. Fatigue degradation over writing sessions (process-intrinsic).
STATUS: Tested and falsified. See SPECTRUM_HYPOTHESIS_LOG.md

Validates the simulation's glucose depletion model against real fatigue data. Compares how
IKI increases over session duration in real writers (Student Fatigue dataset) vs simulation.

Filters to writers with >=3 sessions and examines within-writer slopes. Original analysis
shows 100% monotonic increase, which is suspicious and may be confounded by session length
variation. Within-writer slopes reveal the true fatigue trajectory independent of session
ordering artifacts.

Paper relevance: grounds the metabolic simulation in empirical fatigue research.

Usage:
    uv run python hypotheses-tested/13_fatigue_trajectory_validation_strict.py \\
        --data-dir data/ -o results/fatigue_trajectory_strict.json
"""
...
def _real_fatigue_stats(records: List[dict]) -> dict:
    ...
    n_participants_total = len(by_participant)
    ...
    return {
        ...
        "n_participants_qualified": n_qualified,
...
def _simulation_stats(n_traces: int = 50, n_events: int = 200, seed: int = 42) -> dict:
    ...
    for i in range(n_traces):
        ...
        if len(windows) < 2:
            continue
        w_means = [sum(wnd) / len(wnd) for wnd in windows]
...
def main() -> None:
    ...
    parser.add_argument("-o", "--output", default="results/fatigue_trajectory_strict.json")
    ...
    _out("Computing real fatigue IKI trajectory (within-writer slopes, >=3 sessions)...")
    real_stats, spearman = _real_fatigue_stats(records)
    _out(f"  Total participants        : {real_stats['n_participants_total']}")
    _out(f"  Participants (>=3 sessions): {real_stats['n_participants_qualified']}")
    ...
    result = {
        ...
        "minimum_sessions_per_writer": 3,
        "hypothesis": "Fatigue degradation (spectrum test)",
    }
